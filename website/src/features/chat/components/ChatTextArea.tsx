...
export const ChatTextArea = () => {
    const { input, setInput, sendMessage, isLoading, conversationId, setConversationId } =
        useChatContext();
    ...
    const onSelectFile = useCallback(
        (file: FileResponse | ConversationFile) => {
            ...
            const fileId = "id" in file ? file.id : (file as { _id: string })._id;
            if (
                !stagedFiles.find((f) => ("id" in f ? f.id : (f as { _id: string })._id) === fileId)
            ) {
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (currentId) {
            ...
            uploadFileMutation.mutate(
                { file, conversation_id: currentId },
                {
                    onSuccess: (data) => {
        ...
        <div className="p-3 md:p-4 bg-(--color-bg-elevated) z-10 border-t border-(--color-border)">
            <div className="flex flex-wrap gap-2 mb-2 md:mb-3">
                {uploadingFiles.map((file) => (
                    ...
                    >
                        ...
                        <div className="flex flex-col min-w-0 flex-1">
                            <span className="text-(--color-text-secondary) font-medium truncate max-w-[120px]">
                                {file.name}
                            </span>
                ...
                {stagedFiles.map((file) => (
                    ...
                    >
                        ...
                        <span className="text-(--color-text-primary) font-medium truncate max-w-[140px]">
                            {file.name}
                        </span>
                        <button
                            type="button"
            ...
            >
                ...
                <textarea
                    ...
                    autoFocus
                    className="flex-1 max-h-[160px] min-h-[24px] py-2.5 px-2 text-base bg-transparent outline-none resize-none text-(--color-text-primary) overflow-y-auto"
                    rows={1}
                />
