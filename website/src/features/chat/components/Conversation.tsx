...
import {
    ...
    type UpdateConversationDialogRef,
} from "./UpdateConversationDialog";
...
export const Conversation = ({ title, isSharedView = false, handleFork }: ConversationProps) => {
    const { messages, isLoading, error, clearError, conversationId, currentConversation } =
        useChatContext();
    ...
    const { isDesktopFilesOpen, isMobileFilesOpen, setMobileFilesOpen, toggleFiles } =
        useFilesSidebar();
                            ...
                            >
                                ...
                                {messages.map((msg, i) => {
                                    ...
                                    const isLastAssistant =
                                        !isMe && isLoading && i === messages.length - 1;
                                    ...
                                    if (!isMe && !hasVisibleContent && !isLastAssistant)
                                        return null;
                                    ...
                                    return (
                                        ...
                                        >
                                            <div
                                                className={cn(
                                                    "max-w-[85%] lg:max-w-[75%] flex flex-col",
                                                        ...
                                                        {!hasVisibleContent && isLastAssistant ? (
                                                            ...
                                                            msg.parts.map((part, partIdx) => {
                                                                            ...
                                                                            {isMe ? (
                                                                              ...
                                                                              isLastAssistant ? (
                                                                                <div className="flex gap-1 items-center h-[21px] text-(--color-text-tertiary)">
                                                                                    <span className="animate-bounce [animation-delay:-0.3s] text-lg leading-none">
                                                                                        .
                                                                                    </span>
                                                                                    <span className="animate-bounce [animation-delay:-0.15s] text-lg leading-none">
                                                                                        .
                                                                                    </span>
                                                                                    <span className="animate-bounce text-lg leading-none">
                                                                                        .
                                                                                    </span>
                                                                                </div>
                                                                            ) : (
                                                                ...
                                                                if (
                                                                    ...
                                                                    const itIsImage = isImage(
                                                                        part.file.name,
                                                                        part.media_type,
                                                                    );
                                                                    const itIsAudio = isAudio(
                                                                        part.file.name,
                                                                        part.media_type,
                                                                    );

                                                                    if (itIsImage) {
                                                                        const imageUrl = withCDN(
                                                                            part.file.url,
                                                                        );
                                                                        return (
                                                                            <div
                                                                                key={partIdx}
                                                                                className="my-2 max-w-full overflow-hidden rounded-lg border border-(--color-border) shadow-sm bg-(--color-bg-subtle)"
                                                                            >
                                                                                <img
                                                                                    src={imageUrl}
                                                                                    alt={
                                                                ...
                                                                if (
                                                                    ...
                                                                    return (
                                                                        <div
                                                                            key={partIdx}
                                                                            className="w-full my-2 text-(--color-text-tertiary) text-[13px] border-l-2 border-(--color-border) pl-3 italic bg-black/2 cursor-default select-none"
                                                                        >
                                                                ...
                                                                if (
                                                                    part.type ===
                                                                    "tool-applyImageEffect"
                                                                ) {
                                                                    return (
                                                                        <ToolApplyImageEffect
                                                                            key={partIdx}
                                                                            part={part}
                                                                        />
                                                                    );
