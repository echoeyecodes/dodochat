...
export type ShareDialogRef = {
  open: (data: { conversation_id: string; visibility: 'private' | 'public'; share_token?: string }) => void
}

export const ShareDialog = forwardRef<ShareDialogRef>((_, ref) => {
  const [isOpen, setIsOpen] = useState(false)
  const [data, setData] = useState<{ conversation_id: string; visibility: 'private' | 'public'; share_token?: string } | null>(null)
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-[420px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Anyone with the link can view this conversation and continue it as their own.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6 py-4">
          <div className="flex items-center justify-between p-4 rounded-xl border border-(--color-border) bg-(--color-bg-subtle)">
            <div className="flex flex-col gap-0.5">
              <span className="text-sm font-semibold text-(--color-text-primary)">Public Access</span>
          ...
          {isPublic && data?.share_token && (
            <div className="animate-in fade-in slide-in-from-top-2 duration-300">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-medium text-(--color-text-secondary) ml-1">Shared Link</label>
                <div className="flex gap-2">
                  <div className="flex-1 px-3 py-2 rounded-lg bg-(--color-bg-subtle) border border-(--color-border) text-sm text-(--color-text-primary) font-mono truncate">
                    {shareUrl}
                  </div>
                  <Button
                    size="icon"
                    ...
                    className="shrink-0"
                  >
                    {copied ? <LucideCheck className="w-4 h-4 text-green-500" /> : <LucideCopy className="w-4 h-4" />}
                  </Button>
                </div>
              </div>
