...
import { LucideTrash2, LucideLoader2, LucidePlus } from "lucide-react";
...
export const ConversationList = ({ activeId, onSelect }: ConversationListProps) => {
    ...
    return (
        <div className="w-[280px] shrink-0 flex flex-col bg-(--color-bg) border-r border-(--color-border)">
            <div className="p-4 border-b border-(--color-border) flex flex-col gap-4">
                <div className="flex items-center justify-between">
                    <h2 className="text-[14px] font-semibold text-(--color-text-primary) tracking-tight">
                        Messages
                    </h2>
                    <div className="flex items-center gap-1">
                        ...
                        >
                            <LucidePlus className="w-[18px] h-[18px] stroke-[2px]" />
                        </Link>
                    </div>
                </div>
                <div className="flex items-center gap-2 px-3 py-2 rounded-md bg-(--color-bg-elevated) border border-(--color-border)">
            ...
            <div className="flex-1 overflow-y-auto p-2 scrollbar-hide">
                ...
                {isError && (
                    <ConversationListError error={error as Error} onRetry={() => refetch()} />
                )}
                ...
                {displayItems.map((conv) => (
                    ...
                    >
                        <div className="w-10 h-10 rounded-full flex items-center justify-center font-semibold text-[13px] shrink-0 bg-(--color-bg-muted) text-(--color-text-secondary)">
