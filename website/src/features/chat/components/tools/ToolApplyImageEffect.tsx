...
type ToolApplyImageEffectPart = Extract<UIMessagePart<Record<string, unknown>, ChatTools>, { type: 'tool-applyImageEffect' }>;
    ...
    <div className="bg-(--color-bg-muted) border border-(--color-border) rounded-lg p-3 my-2 text-[12px] font-mono w-full min-w-[240px]">
      ...
      {p.state === 'input-streaming' || p.state === 'input-available' ? (
        ...
        <div className="space-y-2">
          <div className="text-(--color-text-secondary) italic mb-1">{p.output.message}</div>
