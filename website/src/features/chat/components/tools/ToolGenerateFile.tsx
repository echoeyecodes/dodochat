...
type ToolGenerateFilePart = Extract<UIMessagePart<Record<string, unknown>, ChatTools>, { type: 'tool-generateFile' }>;
...
export const ToolGenerateFile: React.FC<ToolGenerateFileProps> = ({ part: p }) => {
    const isPdf = p.state === 'output-available' ? p.output.file_url.endsWith('.pdf') : p.input?.format === 'pdf'
        ...
        <div className="my-2.5 w-full max-w-[320px]">
            ...
            >
                <div className={cn(
                    ...
                    isPdf ? "bg-red-500/10 text-red-500" : "bg-(--color-accent)/10 text-(--color-accent)"
                )}>
                ...
                <div className="flex-1 min-w-0 pr-1 text-left">
                    ...
                    <div className="flex items-center gap-2">
                        <span className="text-[10px] text-(--color-text-tertiary) font-medium uppercase tracking-tight">
                            {p.state === 'output-available' && p.output.size ? `${formatFileSize(p.output.size)} • ` : ''}
