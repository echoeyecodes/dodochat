    ...
    <div className="min-h-screen bg-(--color-bg) flex flex-col selection:bg-(--color-accent-subtle) selection:text-(--color-text-primary) grain">
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-(--color-bg)/80 backdrop-blur-md border-b border-(--color-border-subtle)">
        <div className="max-w-[700px] mx-auto px-6 h-16 flex items-center justify-between">
          <Link to="/" className="text-[14px] font-bold tracking-tight text-(--color-text-primary)">
            DodoChat
          </Link>
          <div className="flex items-center gap-6">
            <Button asChild variant="ghost" size="sm" className="h-8 px-4 text-[12px] font-medium transition-all">
        ...
        <div className="max-w-[700px] mx-auto flex flex-col gap-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-6 text-[12px] font-medium text-(--color-text-quaternary)">
              <a href="https://github.com/echoeyecodes/dodochat" target="_blank" rel="noreferrer" className="hover:text-(--color-text-primary)">GitHub</a>
              <Link to="/privacy" className="hover:text-(--color-text-primary)">Privacy</Link>
              <Link to="/terms" className="hover:text-(--color-text-primary)">Terms</Link>
            </div>
          </div>
        </div>
      </footer>
