...
export const forkConversation = async ({ user_id, share_token }: { user_id: string; share_token: string }) => {
    const original = await Conversation.findOne({ share_token, visibility: CONVERSATION_VISIBILITY.PUBLIC }).lean();
