...
const chat = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        const newUserMessages = messages
            ...
            .map(
                (m: ChatInput["messages"][number]): Message => ({
                    id: m.id,
                    role: m.role,
                    parts: m.parts,
                    created_at: new Date(),
                ...
                parts: (
                    await Promise.all(
                        m.parts
                            ...
                            .map(async (p) => {
                                const partWithFile = p as { type: string; file?: { id: string } };
                                if (p.type === "file" && partWithFile.file?.id) {
                                    const fileRecord = await mediaRepository.getFileById(
                                        partWithFile.file.id,
                                    );
                                    ...
                                    if (
                                        ...
                                        try {
                                            const buffer = await storageService.get(
                                                fileRecord.path,
                                            );
                                            collectedMedia.push({
                                                msgIdx,
                                                ...
                                                type: fileRecord.type.startsWith("image/")
                                                    ? "image"
                                                    : "file",
                                            });
                                        } catch (err) {
                                            console.error(
                                                "Failed to read media file from storage:",
                                                err,
                                            );
                                        }
                                        return null;
            ...
            if (modelMsg && modelMsg.role === "user") {
                const mediaPart =
                    media.type === "image"
                        ? {
                              type: "image" as const,
                              image: new Uint8Array(media.buffer),
        ...
        const files = conversationId
            ? await mediaRepository.getFilesByConversationId(conversationId)
            : [];
        ...
        if (lastUserText && conversationId && files.length > 0) {
            ...
            const relevantChunks = await mediaRepository.findRelevantChunks(
                conversationId,
                embedding,
            );
        ...
        const systemPrompt = [
            ...
            "Spotify Playlist Tool: If the user asks to 'make a playlist', 'create a spotify playlist', or similar, you MUST use the createPlaylist tool.",
            "- You should read the conversation history to gather the songs (title and artist) the user wants. If the user just says 'make a playlist of these', look at the previously suggested songs/results.",
            "- You can also use randomSongs to find songs first if the user just asks for a 'playlist of [genre]'.",
            "- The tool will return a Spotify URL and a success message upon completion.",
            ...
            context
                ...
                : fileListStr
                  ? "The user is asking about the documents, but no specific relevant content was found via vector search. Acknowledge the files exist and ask for more specific questions if needed."
                  : "",
        ]
            .filter(Boolean)
            .join("\n\n");
        ...
        const result = streamText({
            ...
            system: systemPrompt,
            tools: getAllTools(authReq.user_id!),
            stopWhen: stepCountIs(5),
            onFinish: async (event) => {
                const consolidatedParts: Message["parts"] = [];
...
const searchConversations = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const conversations = await conversationRepository.searchConversations({
            user_id: authReq.user_id!,
            query,
        });
        return sendResponse({ res, status: HTTP_STATUS_CODES.OK })(conversations);
...
const shareConversation = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const conversation = await conversationRepository.toggleSharing({
            user_id: authReq.user_id!,
            id,
            visibility,
        });
...
const forkConversation = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const authReq = req as AuthRequest;
        const token = req.params.token as string;
        const conversation = await conversationRepository.forkConversation({
            user_id: authReq.user_id!,
            share_token: token,
        });
        return sendResponse({ res, status: HTTP_STATUS_CODES.CREATED })(conversation);
