...
export const getCurrentUser = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        userWithoutPassword.gemini_api_key = userWithoutPassword.gemini_api_key
            ? safeDecryptGeminiKey(userWithoutPassword.gemini_api_key)
            : undefined;
...
export const updateCurrentUser = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const user = (await userRepository.updateUser(req.user_id!, updates)) as User;
        const { password: _, ...userWithoutPassword } = user;

        if (userWithoutPassword.gemini_api_key) {
            userWithoutPassword.gemini_api_key = safeDecryptGeminiKey(
                userWithoutPassword.gemini_api_key,
            );
        }
