...
import { ACCESS_TOKEN_EXPIRES_IN, ACCESS_TOKEN_MAX_AGE, ACCESS_TOKEN_SECRET, REFRESH_TOKEN_MAX_AGE, REFRESH_TOKEN_SECRET } from '../constants/index';
...
const generateTokens = (userId: string) => {
    const access_token = jwt.sign({ sub: userId }, ACCESS_TOKEN_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRES_IN });
...
export const login = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const { access_token, refresh_token, access_token_expires_at } = generateTokens(user!._id.toString());
...
export const refreshAccessToken = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const existingToken = await authRepository.getAuthToken({ refresh_token: hashToken(refresh_token) }).catch((error) => {
        ...
        const { access_token: newAccessToken, access_token_expires_at } = generateTokens(decoded.sub);
